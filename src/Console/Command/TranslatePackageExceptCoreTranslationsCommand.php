<?php
namespace XanUtility\Console\Command;
...
class TranslatePackageExceptCoreTranslationsCommand extends Command
{
    protected function configure()
    {
        ...
        $this
            ->setName('c5:package:translate-except-core')
            ->setAliases([
                'c5:package-translate-except-core',
                'c5:translate-package-except-core',
            ])
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (is_dir($p) || !$vsh->handle($p)) {
            ...
            $pkgSvc = $this->app->make(PackageService::class);
            foreach ($pkgSvc->getAvailablePackages(false) as $pkg) {
        ...
        foreach ($locales as $locale) {
            ...
            $output->writeln("- Checking Core Translations on locale $locale");
            if (!file_exists(DIR_LANGUAGES . "/$locale/LC_MESSAGES/messages.mo")) {
                ...
                $output->writeln("-- $removedMessages Translations Removed, already translated by Core.");
            }
